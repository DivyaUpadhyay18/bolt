...
print(f"Processing {frame_count} frames with OPTIMIZED detection...")

hole_tracks = {}
next_hole_id = 1
proximity_threshold = 65

# Optimized color ranges - balanced for subtle cluster detection
color_ranges = {
...
def get_multi_color_clusters(frame):
    """Extract multi-color clusters efficiently"""
    ...
    for contour in contours:
        ...
        # Check color composition (require 2+ colors, but be lenient)
        colors_present = set()
        ...
        # Accept clusters with 1+ colors (very relaxed)
...
# Filter: keep holes appearing in 2+ frames (lenient)
valid_holes = {}
for hole_id, track in hole_tracks.items():
    if len(track) >= 2:
