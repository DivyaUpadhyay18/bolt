...
print(f"Processing {frame_count} frames - balanced detection for all cluster types...")

hole_tracks = {}
next_hole_id = 1
proximity_threshold = 60

# Wider color ranges to catch subtle/disturbance clusters
color_ranges = {
    'purple': [(95, 0, 0), (180, 255, 255)],
    'red': [(0, 20, 20), (45, 255, 255)],
    'gray': [(0, 0, 20), (180, 160, 220)]
}

def get_multi_color_clusters(frame):
    """Extract multi-color clusters with wider ranges"""
    ...
    # Larger kernel to connect disturbance pixels better
    ...
    for contour in contours:
        ...
        # Relaxed area range to catch subtle clusters
        if area < 50 or area > 22000:
            continue
        ...
        aspect_ratio = w / h
        # Relaxed aspect ratio for distorted clusters
        if aspect_ratio < 0.3 or aspect_ratio > 3.3:
            continue
        
        # Check color composition - require 2+ colors
        colors_present = set()
        ...
        # Require 2+ colors but accept multi-color disturbance clusters
...
for frame_idx in range(frame_count):
    ...
    clusters = get_multi_color_clusters(frame)
...
# Relaxed temporal filter - keep holes appearing in 2+ frames
valid_holes = {}
for hole_id, track in hole_tracks.items():
    if len(track) >= 2:
