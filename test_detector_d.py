...
print(f"DETECTOR D: Combo - Wide Colors + Large Kernel + 2+ colors + 1+ frames")
...
def get_clusters(frame):
    ...
    # Large kernel to connect disturbance pixels
...
# Only 1+ frames (most relaxed temporal)
valid_holes = {hole_id: track for hole_id, track in hole_tracks.items() if len(track) >= 1}
...
print(f"\n{'='*60}")
print(f"DETECTOR D RESULT: {len(final_holes)} HOLES")
print(f"{'='*60}\n")
